...
class TestMultiFilter extends MultiFilter {
  ...
  build() {
    ...
    return this.buildAndCache(["compile1", "compile2"], function(
      token,
      outputDirectoryPath
    ) {
  ...
  compile1(outputDirectoryPath) {
    ...
    return [this.inputPaths[0] + "/in1.txt", this.inputPaths[0] + "/dep1.txt"];
  }
  ...
  describe("usage errors", () => {
    ...
    describe("tokens (input files)", () => {
      it("requires tokens to be strings", () => {
        return expect(
          fixture.build(new FailingPlugin([null], () => {}))
        ).to.be.rejectedWith(/Expected a string/);
      });

      it("rejects duplicate tokens", () => {
        return expect(
          fixture.build(new FailingPlugin(["foo.js", "foo.js"], () => {}))
